<template>
    <!----统计局---投入品使用情况 添加编辑详情---->
    <div id="statistInputUsageDetail"  class="pageContent" v-loading="loading">
                ...
                <el-collapse-item name="1">
                    <template slot="title">
                        <div class="titleContent"><i class="collapseDownIcon el-icon-caret-bottom" :class="[nameFlag.indexOf('1') >=0?'el-icon-caret-bottom':'el-icon-caret-right']"></i>
                            <span>基本信息</span>
                            <span class="titleRight">总合计：<span class="valueStyle">{{ruleForm.totalarea}}</span> 吨</span>
                        </div>
                    </template>
                            ...
                            <el-collapse-item name="1-2">
                                ...
                                <el-form :model="ruleForm" status-icon label-width="100px" class="demo-ruleForm"> 
                                    <el-form-item label="地膜覆盖面积：" class="formItem">
                                        <el-input v-model="ruleForm.dmfwmj" onkeyup="this.value = this.value.replace(/[^\d.]/g,'');"  :disabled="urlType" placeholder="地膜覆盖面积"></el-input> (亩)
                                    </el-form-item> 
                                    
                                    <el-form-item label="地膜使用量：" class="formItem">
                                        <el-input v-model="ruleForm.dmsyl" onkeyup="this.value = this.value.replace(/[^\d.]/g,'');"  :disabled="urlType" placeholder="地膜使用量"></el-input> (吨)
                                    </el-form-item> 
                                    <div style="clear:both;"></div>
                                </el-form>
                                </div>
...
<script>
//统计局--投入品使用情况 和 土肥站---投入品使用情况 使用统一的接口
import {townshipOption} from "@/assets/js/commonOptions";
import { inputUsageDetail, inputUsageUpdate,inputUsageInsert } from '@/utils/api/tuFeiZhan/inputUsage'; 
import {handleCofirm} from '@/utils/messageBox/confirm';
import {getData} from '@/utils/nowTime';
import CustomForm from '@/components/customForm/CustomForm';
export default {
    name:'statistInputUsageDetail',
    data() {
        return {
    ...
    watch:{
        ...
        testDataNew: {
            handler(newVal) {
                ...
                this.ruleForm.totalarea = hfsylsw + nyslbms + other;
           },
