<template>
    <!--产业办---无公害企业产品数据详情-->
    <div class="doHarmlessProductsEnterDetail pageContent" v-loading="loading">
        <section class="sectionContent">
            <div class="miniNav">{{miniNav}}</div>
            <el-collapse  v-model="activeNames" @change="handleChange">
                <el-collapse-item name="1">
                    <template slot="title">
                        <div><i class="collapseDownIcon el-icon-caret-bottom" :class="[nameFlag.indexOf('1') >=0?'el-icon-caret-bottom':'el-icon-caret-right']"></i>企业信息</div> <!---->
                    </template>
                    <div class="formContent">
                        <el-form :model="ruleForm" status-icon :rules="rules" ref="ruleForm1" label-width="100px" class="demo-ruleForm">
                            ...
                            <el-form-item label="行业：" prop="industry" class="formItem formSelect" >
                                <el-select v-model="ruleForm.industry" filterable clearable :disabled="urlType" placeholder="请选择">
                                    <el-option
                            ...
                            <div style="clear:both;"></div>
                            <!-- <el-form-item label="联系地址：" prop="address" class="formItem">
                                <el-input v-model="ruleForm.address" :disabled="urlType" placeholder="请输入联系地址"></el-input>
                            </el-form-item> -->
                            ...
                            <el-form-item label="身份证号码：" prop="idcard" class="formItem">
                                <el-input v-model="ruleForm.idcard" :disabled="urlType" placeholder="请输入身份证"></el-input>
                            </el-form-item> 
                        </el-form>
                    </div>
                </el-collapse-item>
                <el-collapse-item name="2">
                    <template slot="title">
                        <div><i class="collapseDownIcon" :class="[nameFlag.indexOf('2') >=0?'el-icon-caret-bottom':'el-icon-caret-right']"></i>产品信息</div>
                    </template>
                    <div class="formContent">
                        <el-form :model="ruleForm" status-icon :rules="rules" ref="ruleForm2" label-width="100px" class="demo-ruleForm">
                            <el-form-item label="产品：" prop="productName" class="formItem">
                                <el-input v-model="ruleForm.productName" :disabled="urlType" placeholder="请输入产地监测面积"></el-input>
                            </el-form-item>
                            <el-form-item label="规模：" prop="scale" class="formItem">
                                <el-input v-model="ruleForm.scale" onkeyup="this.value = this.value.replace(/[^\d.]/g,'');" :disabled="urlType" placeholder="请输入规模"></el-input>（公顷）
                            </el-form-item>
                            <div style="clear:both;"></div>
                            <el-form-item label="产量：" prop="yield" class="formItem">
                                <el-input v-model="ruleForm.yield" onkeyup="this.value = this.value.replace(/[^\d.]/g,'');" :disabled="urlType" placeholder="请输入产量"></el-input>（吨）
                            </el-form-item>
                            ...
                            <el-form-item label="结束时间：" prop="endTime" class="formItem">
                                <el-date-picker 
...
import {townshipOption, industryOption} from "@/assets/js/commonOptions";
import {getData} from '@/utils/nowTime';
import CustomForm from '@/components/customForm/CustomForm';
//质检办--无公害企业产品数据 ，产业办--无公害企业产品数据 使用同一套接口
import { qualityPollutionDetail, qualityPollutionUpdate, qualityPollutionInsert } from '@/utils/api/qualityInspection/qualityPollutionProducts'; 
...
export default {
    data() {
        return {
            ...
            rules: {
                ...
                varieties:[
                      { required: true, message: '请选择种植品种',trigger: 'blur' }
                ],
                productName:[
                ...
                idcard:[
                     { required: false},
                     {validator: validate.FormValidate.Form().validateIdCard,trigger: 'blur'}
                ]
            },
            townshipOption:townshipOption,
    ...
    methods:{
        ...
        //初始化页面数据
        initData(){
            this.getDataDetail(this.id); //调用初始化接口
        },
        //详情接口，获取详情数据
        async getDataDetail(id){
            ...
            let res = await qualityPollutionDetail({id});
        ...
        async updateData(){
            ...
            let res = await qualityPollutionUpdate(updateObj);
             if(res.data.status== 0){
                this.$message({
                    message: '编辑成功',
                    type: 'success'
        ...
        async addData(){
            ...
            let res = await qualityPollutionInsert(addObj);
             if(res.data.status== 0){
                this.$message({
                    message: '新增成功',
                    type: 'success'
