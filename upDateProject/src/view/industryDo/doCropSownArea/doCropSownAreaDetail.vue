<template>
    <!----产业办---农作物播种面积 添加编辑详情---->
    <div id="doCropSownAreaDetail" class="pageContent" v-loading="loading">
                ...
                <el-collapse-item name="5">
                    <template slot="title">
                        <div class="titleContent">
                            ...
                            <span>其他类</span>
                            <span class="titleRight">合计：<span class="valueStyle">{{ruleForm.otherCrops}}</span> 亩</span>
                        </div> 
                    </template>
                    <div class="formContent">
                        <el-form :model="ruleForm" status-icon label-width="100px" class="demo-ruleForm">
...
<script>
import { doCropSownAreaDetail,doCropSownAreaUpdate, doCropSownAreaInsert } from '@/utils/api/industryDo/doCropSownArea' //农经办---农作物播种面积
import {townshipOption} from "@/assets/js/commonOptions";
import {handleCofirm} from '@/utils/messageBox/confirm';
import {getData} from '@/utils/nowTime';
import CustomForm from '@/components/customForm/CustomForm';
export default {
    name:'doCropSownAreaDetail',
    data() {
        return {
            ...
            activeNames: ['1','2','2-1','3','4','5'], //默认折叠板全部打开
            nameFlag:[],
            miniNav:'', //左上角导航
            id:'', //数据的id
    ...
    watch:{
        ...
        testDataNew: {
            handler(newVal) {
                ...
                foodstuff =parseFloat(newVal['beans']) +parseFloat(newVal['tubers'])+parseFloat(newVal['rice'])+parseFloat(newVal['wheat'])+parseFloat(newVal['corn']);
                grain = parseFloat(newVal['rice'])+parseFloat(newVal['wheat'])+parseFloat(newVal['corn']);
                oil =parseFloat(newVal['peanut']) +parseFloat(newVal['rape'])+parseFloat(newVal['sesame']);
                melonsFruitsm = parseFloat(newVal['watermelon']) +parseFloat(newVal['muskmelon'])+parseFloat(newVal['strawberry']);
                otherCrops = parseFloat(newVal['cotton']) +parseFloat(newVal['flax'])+parseFloat(newVal['sugar'])+parseFloat(newVal['cmedicine'])+parseFloat(newVal['vegetables']);
                console.log(grain,)
                ...
                this.ruleForm.totalarea = foodstuff + grain + oil + melonsFruitsm + otherCrops;
           },
    ...
    methods:{
        ...
        async getDataDetail(){
            ...
            let res = await doCropSownAreaDetail({id:this.id});
        ...
        async updateData(){
            ...
            let res = await doCropSownAreaUpdate(updateObj);
             if(res.data.status== 0){
                this.$message({
                    message: '编辑成功',
                    type: 'success'
        ...
        async addData(){
            ...
            let res = await doCropSownAreaInsert(addObj);
             if(res.data.status== 0){
                this.$message({
                    message: '新增成功',
                    type: 'success'
