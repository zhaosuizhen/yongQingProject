<template>
    <!--产业办——有机食品企业产品数据新增、编辑、详情页面-->
    <div id="doOrganicFoodEnterDetail" class="pageContent" v-loading="loading">
        <section class="sectionContent">
            ...
            <el-collapse  v-model="activeNames" @change="handleChange">
                ...
                <el-collapse-item name="2">
                    <template slot="title">
                        <div><i class="collapseDownIcon" :class="[nameFlag.indexOf('2') >=0?'el-icon-caret-bottom':'el-icon-caret-right']"></i>种植信息</div>
                    </template>
                    <div class="formContent">
                        <el-form :model="ruleForm" status-icon :rules="rules" ref="ruleForm2" label-width="100px" class="demo-ruleForm">
                            <el-form-item label="种植品种：" prop="varieties" class="formItem">
                                 <el-input v-model="ruleForm.varieties" :disabled="urlType" placeholder="请输入种植品种"></el-input>
                            </el-form-item>
                            <el-form-item label="产地监测面积：" prop="checkArea" class="formItem">
                                <el-input v-model="ruleForm.checkArea" onkeyup="this.value = this.value.replace(/[^\d.]/g,'');" :disabled="urlType" placeholder="请输入产地监测面积"></el-input>（亩）
                            </el-form-item>
                            <div style="clear:both;"></div>
...
<script>
//与质检办--有机食品企业产品数据同一个接口
import { qualityOrganicDetail, qualityOrganicUpdate,qualityOrganicInsert } from '@/utils/api/qualityInspection/qualityOrganicProducts'; 
...
export default {
    ...
    methods:{
        ...
        async getDataDetail(){
            ...
            let res = await qualityOrganicDetail({id:this.id});
        ...
        async updateData(){
            ...
            let res = await qualityOrganicUpdate(updateObj);
             if(res.data.status== 0){
                this.$message({
                    message: '编辑成功',
                    type: 'success'
        ...
        async addData(){
            ...
            let res = await qualityOrganicInsert(addObj);
             if(res.data.status== 0){
                this.$message({
                    message: '新增成功',
                    type: 'success'
