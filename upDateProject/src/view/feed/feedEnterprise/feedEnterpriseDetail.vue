<template>
<!--质检办---经营主体数据详情-->
    <div id="feedEnterpriseDetail" class="pageContent" v-loading="loading">
                    ...
                    <div class="formContent">
                        <el-form :model="ruleForm" status-icon :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
                            <el-form-item label="企业名称：" prop="enterpriseName" class="formItem autocomplete">
                                <el-input v-model="ruleForm.enterpriseName" :disabled="urlType" placeholder="请输入企业名称"></el-input>
                            </el-form-item>
                            <el-form-item label="注册资金：" prop="registeredCapital" class="formItem autocomplete">
                                <el-input v-model="ruleForm.registeredCapital" :disabled="urlType" placeholder="请输入注册资金"></el-input>
                            </el-form-item>
                            <div style="clear:both;"></div>
                            <el-form-item label="法定代表人："  prop="legalRepresentative" class="formItem">
                                <el-input v-model="ruleForm.legalRepresentative" :disabled="urlType" placeholder="请输入法定代表人"></el-input>
                            </el-form-item>
                            ...
                            <el-form-item label="占地面积：" class="formItem">
                                <el-input v-model="ruleForm.areaCovered" :disabled="urlType" placeholder="请输入占地面积"></el-input> (亩)
                            </el-form-item>
                            <div style="clear:both;"></div>
                            
                            
                            <el-form-item label="经营范围：" class="formItem-textarea">
                                <el-input v-model="ruleForm.businessScope" type="textarea" :rows="5" :disabled="urlType" class="formTextarea" placeholder="请输入经营范围"></el-input>
                            </el-form-item>
                            
                            <el-form-item label="详细地址：" class="formItem" prop="address">
                                <el-input v-model="ruleForm.address" :disabled="urlType" placeholder="请输入经营范围"></el-input>
                            </el-form-item>
...
<script>
import { feedEnterpriseDetail,feedEnterpriseUpdate, feedEnterpriseInsert } from '@/utils/api/feed/feedEnterprise' //饲料办---饲料企业
...
export default {
    ...
    data() {
        return {
            ...
            ruleForm: {
                ...
                townShip:'',//所属乡镇
                updateTime:'',//更新时间
                certificateValidityStartTime: '',//证件有效期开始时间
                certificateValidityEndTime: '',//证件有效期结束时间
            },
            ...
            subjectTypAeOption:subjectTypAeOption, //经营类别
            rules: {
                legalRepresentative:[
    ...
    methods:{
        //改变时间
        choiceTime(){
            this.ruleForm.certificateValidityStartTime = this.time[0]
            this.ruleForm.certificateValidityEndTime = this.time[1]
        },
        //自定义表单数值更改，父组件同步
        ...
        async getDataDetail(){
            ...
            let res = await feedEnterpriseDetail({id:this.id});
             if(res.data.status== 0){
                 console.log(res.data.entitys)
                 if(res.data.entitys.certificateValidityStartTime && res.data.entitys.certificateValidityEndTime){
                     this.time = [res.data.entitys.certificateValidityStartTime, res.data.entitys.certificateValidityEndTime]
                 }
        ...
        async updateData(){
            ...
            let res = await feedEnterpriseUpdate(updateObj);
             if(res.data.status== 0){
                this.$message({
                    message: '编辑成功',
                    type: 'success'
        ...
        async addData(){
            ...
            let res = await feedEnterpriseInsert(addObj);
             if(res.data.status== 0){
                this.$message({
                    message:'新增成功',
                    type: 'success'
