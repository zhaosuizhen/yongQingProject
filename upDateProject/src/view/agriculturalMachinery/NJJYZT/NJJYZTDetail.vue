                ...
                <el-collapse-item name="1">
                    ...
                    <div class="formContent">
                        <el-form :model="ruleForm" status-icon :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
                            <el-form-item label="合作社名称" prop="enterpriseName" class="formItem">
                                <el-input v-model="ruleForm.enterpriseName" :disabled="urlType" placeholder="请输入合作社名称"></el-input>
                            </el-form-item>
                            <el-form-item label="注册日期" prop="registeredDate" class="formItem">
                                <el-date-picker
                                    :disabled="urlType"
                                    v-model="ruleForm.registeredDate" 
                                    class="pickerWidth" 
                                    type="date" 
                            ...
                            <div style="clear:both;"></div>
                            <el-form-item label="工商注册号" prop="registeredNum" class="formItem">
                                <el-input v-model="ruleForm.registeredNum" :disabled="urlType" placeholder="请输入工商注册号"></el-input>
                            </el-form-item>
                            <el-form-item label="邮政编码" prop="postCode" class="formItem">
                                <el-input v-model="ruleForm.postCode" onkeyup="this.value = this.value.replace(/[^\d.]/g,'');" maxlength="11" :disabled="urlType" placeholder="请输入邮政编码"></el-input>
                            </el-form-item>
                            <div style="clear:both;"></div>
                            ...
                            <!-- <el-form-item label="合作社级别" prop="hzLevel" class="formItem formSelect">
                                <el-select v-model="ruleForm.hzLevel" filterable clearable :disabled="urlType" placeholder="请选择合作社级别">
                                    <el-option
                                    v-for="item in hzsLevel"
                                    :key="item.id"
                                    :label="item.name"
                                    :value="item.value">
                                    </el-option>
...
<script>
import { NJJYZTDetail,NJJYZTInsert, NJJYZTUpdate } from '@/utils/api/agriculturalMachinery/NJJYZT' //畜牧股---生猪养殖
import {townshipOption,hzsLevel} from "@/assets/js/commonOptions";
...
export default {
    data() {
        return {
            ...
            rules: {
                ...
                hzLevel:[
                     { required: true, message: '请选择合作社级别',trigger: 'blur' }
                ],
                address:[
    ...
    methods:{
        ...
        async getDataDetail(){
            ...
            let res = await NJJYZTDetail({id:this.id});
        ...
        async updateData(){
            ...
            let res = await NJJYZTUpdate(updateObj);
             if(res.data.status== 0){
                this.$message({
                    message: '编辑成功',
                    type: 'success'
        ...
        async addData(){
            ...
            let res = await NJJYZTInsert(addObj);
             if(res.data.status== 0){
                this.$message({
                    message:'新增成功',
                    type: 'success'
