                        ...
                        <div class="formContent">
                            <el-form :model="ruleForm" status-icon label-width="100px" class="demo-ruleForm">
                                ...
                                <el-form-item label="休闲渔业" class="formItem">
                                    <el-input v-model="ruleForm.recreationalFishery" onkeyup="this.value = this.value.replace(/[^\d.]/g,'');"  :disabled="urlType" placeholder="请输入休闲渔业产值"></el-input> （万元）
                                </el-form-item>
                                <el-form-item label="其它" class="formItem">
...
<script>
//统计局---渔业产值 接口
import { fisheryYieldDetail,fisheryYieldUpdate, fisheryYieldInsert } from '@/utils/api/fisheriesAdministration/fisheryYield' 
...
export default {
    ...
    methods:{
        ...
        async getDataDetail(){
            ...
            let res = await fisheryYieldDetail({id:this.id});
        ...
        async updateData(){
            ...
            let res = await fisheryYieldUpdate(updateObj);
             if(res.data.status== 0){
                this.$message({
                    message: '编辑成功',
                    type: 'success'
        ...
        async addData(){
            ...
            let res = await fisheryYieldInsert(addObj);
             if(res.data.status== 0){
                this.$message({
                    message: '新增成功',
                    type: 'success'
