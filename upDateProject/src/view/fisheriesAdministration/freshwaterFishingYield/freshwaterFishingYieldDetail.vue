            ...
            <el-collapse  v-model="activeNames" @change="handleChange">
                <el-collapse-item name="0">
                    ...
                    <el-form :model="ruleForm" status-icon :rules="rules" ref="ruleForm" label-width="1000px" class="demo-ruleForm">
                        ...
                        <el-form-item label="鱼类" class="formItem" style="width:50%;margin-top:10px">
                            <el-input v-model="ruleForm.fish" onkeyup="this.value = this.value.replace(/[^\d.]/g,'');"  :disabled="urlType" placeholder="请输入鱼类数量"></el-input> （吨）
                        </el-form-item>
                        <div style="clear:both;"></div>
                        <el-form-item label="贝类" class="formItem" style="width:50%;margin-top:10px">
                            <el-input v-model="ruleForm.shellfish" onkeyup="this.value = this.value.replace(/[^\d.]/g,'');"  :disabled="urlType" placeholder="请输入贝类数量"></el-input> （吨）
                        </el-form-item>
                        <el-form-item label="藻类" class="formItem" style="width:50%;margin-top:10px">
                            <el-input v-model="ruleForm.alga" onkeyup="this.value = this.value.replace(/[^\d.]/g,'');"  :disabled="urlType" placeholder="请输入藻类数量"></el-input> （吨）
                        </el-form-item>
                        <div style="clear:both;"></div>
                    </el-form>
                    ...
                    <el-collapse-item name="1">
                        <template slot="title">
                            <div class="titleContent"><i class="collapseDownIcon el-icon-caret-bottom" :class="[nameFlag.indexOf('1') >=0?'el-icon-caret-bottom':'el-icon-caret-right']"></i>
                                <span>甲壳类</span>
                                <span class="titleRight">合计：<span class="valueStyle">{{ruleForm.carapaceTotal}}</span> 吨</span>
                            </div>
                        </template>
                        <div class="formContent">
                            <el-form :model="ruleForm" status-icon label-width="100px" class="demo-ruleForm">
                                ...
                                <el-form-item label="蟹" class="formItem">
                                    <el-input v-model="ruleForm.crab" onkeyup="this.value = this.value.replace(/[^\d.]/g,'');"  :disabled="urlType" placeholder="请输入蟹数量"></el-input> （吨）
                                </el-form-item>
                                <div style="clear:both;"></div>
                            </el-form>
                    ...
                    <el-collapse-item name="2">
                        <template slot="title">
                            <div class="titleContent"><i class="collapseDownIcon el-icon-caret-bottom" :class="[nameFlag.indexOf('2') >=0?'el-icon-caret-bottom':'el-icon-caret-right']"></i>
                                <span>其他类</span>
                                <span class="titleRight">合计：<span class="valueStyle">{{ruleForm.freshwateOther}}</span> 吨</span>
                            </div>
                        </template>
                        <div class="formContent">
                            <el-form :model="ruleForm" status-icon label-width="100px" class="demo-ruleForm">
                                <el-form-item label="丰年虫" class="formItem">
                                    <el-input v-model="ruleForm.shrimpFairy" onkeyup="this.value = this.value.replace(/[^\d.]/g,'');"  :disabled="urlType" placeholder="请输入丰年虫数量"></el-input> （吨）
                                </el-form-item>
                            </el-form>
                        </div>
                    </el-collapse-item>
...
<script>
//统计局---畜产品产量 接口
import { freshwaterFishingYieldDetail,freshwaterFishingYieldUpdate, freshwaterFishingYieldInsert } from '@/utils/api/fisheriesAdministration/freshwaterFishingYield' 
...
export default {
    ...
    watch:{
        ...
        testDataNew: {
            handler(newVal) {
                this.ruleForm.freshwateOther = newVal.shrimpFairy * 1
                this.ruleForm.carapaceTotal = newVal.shrimp * 1 + newVal.crab * 1
                this.ruleForm.allData = newVal.fish * 1 + newVal.shellfish * 1 + newVal.alga * 1 + this.ruleForm.freshwateOther + this.ruleForm.carapaceTotal
           },
    ...
    methods:{
        ...
        async getDataDetail(){
            ...
            let res = await freshwaterFishingYieldDetail({id:this.id});
        ...
        async updateData(){
            ...
            let res = await freshwaterFishingYieldUpdate(updateObj);
             if(res.data.status== 0){
                this.$message({
                    message: '编辑成功',
                    type: 'success'
        ...
        async addData(){
            ...
            let res = await freshwaterFishingYieldInsert(addObj);
             if(res.data.status== 0){
                this.$message({
                    message: '新增成功',
                    type: 'success'
