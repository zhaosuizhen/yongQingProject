...
<script>
import { pesticidesBusinessDetail,pesticidesBusinessInsert, pesticidesBusinessUpdate } from '@/utils/api/enforcementTeam/pesticidesBusiness' //畜牧股---生猪养殖
...
export default {
    ...
    methods:{
        ...
        async getDataDetail(){
            ...
            let res = await pesticidesBusinessDetail({id:this.id});
        ...
        async updateData(){
            ...
            let res = await pesticidesBusinessUpdate(updateObj);
             if(res.data.status== 0){
                this.$message({
                    message: '编辑成功',
                    type: 'success'
        ...
        async addData(){
            ...
            let res = await pesticidesBusinessInsert(addObj);
             if(res.data.status== 0){
                this.$message({
                    message:'新增成功',
                    type: 'success'
