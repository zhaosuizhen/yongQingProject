                    ...
                    <el-collapse-item name="1">
                        <template slot="title">
                            <div  class="titleContent">
                                ...
                                <span>鲜果产量</span>
                                <span class="titleRight">总计：<span class="valueStyle">{{ruleForm.sumYield}}</span> 吨</span>
                            </div> 
                        </template>
                        <div class="formContent">
                            <el-form :model="ruleForm" status-icon :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
                                ...
                                <el-form-item label="苹果" class="formItem">
                                    <el-input v-model="ruleForm.appleYield" onkeyup="this.value = this.value.replace(/[^\d.]/g,'');"  :disabled="urlType" placeholder="请输入苹果数量"></el-input> （吨）
                                </el-form-item>
                                <el-form-item label="桃子" class="formItem">
                                    <el-input v-model="ruleForm.peachYield" onkeyup="this.value = this.value.replace(/[^\d.]/g,'');"  :disabled="urlType" placeholder="请输入桃子数量"></el-input> （吨）
                                </el-form-item>
                                <el-form-item label="梨" class="formItem">
                                    <el-input v-model="ruleForm.pearYield" onkeyup="this.value = this.value.replace(/[^\d.]/g,'');"  :disabled="urlType" placeholder="请输入梨数量"></el-input> （吨）
                                </el-form-item>
                                <el-form-item label="杏" class="formItem">
                                    <el-input v-model="ruleForm.apricotYield" onkeyup="this.value = this.value.replace(/[^\d.]/g,'');"  :disabled="urlType" placeholder="请输入杏数量"></el-input> （吨）
                                </el-form-item>
                                <el-form-item label="葡萄" class="formItem">
                                    <el-input v-model="ruleForm.grapeYield" onkeyup="this.value = this.value.replace(/[^\d.]/g,'');"  :disabled="urlType" placeholder="请输入葡萄数量"></el-input> （吨）
                                </el-form-item>
                                <el-form-item label="其他" class="formItem">
                                    <el-input v-model="ruleForm.otherYield" onkeyup="this.value = this.value.replace(/[^\d.]/g,'');"  :disabled="urlType" placeholder="请输入其他数量"></el-input> （吨）
                                </el-form-item>
...
<script>
//种植业股---鲜果产量 接口
import { freshFruitYieldDetail,freshFruitYieldUpdate, freshFruitYieldInsert } from '@/utils/api/planting/freshFruitYield' 
...
export default {
    ...
    watch:{
        ...
        testDataNew: {
            handler(newVal) {
                console.log(newVal)
                //总和
                this.ruleForm.sumYield = newVal.appleYield * 1 + newVal.grapeYield * 1 + newVal.peachYield * 1 +
                                        newVal.pearYield * 1 + newVal.apricotYield * 1 + newVal.otherYield * 1
           },
    ...
    methods:{
        ...
        async getDataDetail(){
            ...
            let res = await freshFruitYieldDetail({id:this.id});
        ...
        async updateData(){
            ...
            let res = await freshFruitYieldUpdate(updateObj);
             if(res.data.status== 0){
                this.$message({
                    message: '编辑成功',
                    type: 'success'
        ...
        async addData(){
            ...
            let res = await freshFruitYieldInsert(addObj);
             if(res.data.status== 0){
                this.$message({
                    message: '新增成功',
                    type: 'success'
