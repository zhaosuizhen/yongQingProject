                    ...
                    <el-collapse-item name="1">
                        <template slot="title">
                            <div  class="titleContent">
                                ...
                                <span>鲜果产量</span>
                                <span class="titleRight">总计：<span class="valueStyle">{{ruleForm.sumCount}}</span> 亩</span>
                            </div> 
                        </template>
                        <div class="formContent">
                            <el-form :model="ruleForm" status-icon :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
                                ...
                                <el-form-item label="经济林" class="formItem">
                                    <el-input v-model="ruleForm.economicForest" onkeyup="this.value = this.value.replace(/[^\d.]/g,'');"  :disabled="urlType" placeholder="请输入经济林数量"></el-input> （亩）
                                </el-form-item>
                                <el-form-item label="用材林" class="formItem">
                                    <el-input v-model="ruleForm.timberForest" onkeyup="this.value = this.value.replace(/[^\d.]/g,'');"  :disabled="urlType" placeholder="请输入用材林数量"></el-input> （亩）
                                </el-form-item>
                                <el-form-item label="零星植树" class="formItem">
                                    <el-input v-model="ruleForm.sporadicTreePlanting" onkeyup="this.value = this.value.replace(/[^\d.]/g,'');"  :disabled="urlType" placeholder="请输入零星植树数量"></el-input> （百株）
                                </el-form-item>
                                <el-form-item label="育苗面积" class="formItem">
                                    <el-input v-model="ruleForm.seedlingArea" onkeyup="this.value = this.value.replace(/[^\d.]/g,'');"  :disabled="urlType" placeholder="请输入育苗面积数量"></el-input> （亩）
                                </el-form-item>
...
<script>
//种植业股---鲜果产量 接口
import { forestryProductDetail,forestryProductUpdate, forestryProductInsert } from '@/utils/api/planting/forestryProduct' 
...
export default {
    ...
    watch:{
        ...
        testDataNew: {
            handler(newVal) {
                console.log(newVal)
                //总和
                this.ruleForm.sumCount = newVal.economicForest * 1 + newVal.timberForest * 1  + newVal.seedlingArea * 1
           },
    ...
    methods:{
        ...
        async getDataDetail(){
            ...
            let res = await forestryProductDetail({id:this.id});
        ...
        async updateData(){
            ...
            let res = await forestryProductUpdate(updateObj);
             if(res.data.status== 0){
                this.$message({
                    message: '编辑成功',
                    type: 'success'
        ...
        async addData(){
            ...
            let res = await forestryProductInsert(addObj);
             if(res.data.status== 0){
                this.$message({
                    message: '新增成功',
                    type: 'success'
