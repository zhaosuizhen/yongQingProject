                    ...
                    <el-collapse-item name="1">
                        <template slot="title">
                            <div  class="titleContent">
                                ...
                                <span>果园面积</span>
                                <span class="titleRight">总计：<span class="valueStyle">{{ruleForm.sumArea}}</span> 亩</span>
                            </div> 
                        </template>
                        <div class="formContent">
                            <el-form :model="ruleForm" status-icon :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
                                ...
                                <el-form-item label="苹果" class="formItem">
                                    <el-input v-model="ruleForm.appleArea" onkeyup="this.value = this.value.replace(/[^\d.]/g,'');"  :disabled="urlType" placeholder="请输入苹果数量"></el-input> （亩）
                                </el-form-item>
                                <el-form-item label="桃子" class="formItem">
                                    <el-input v-model="ruleForm.peachArea" onkeyup="this.value = this.value.replace(/[^\d.]/g,'');"  :disabled="urlType" placeholder="请输入桃子数量"></el-input> （亩）
                                </el-form-item>
                                <el-form-item label="梨" class="formItem">
                                    <el-input v-model="ruleForm.pearArea" onkeyup="this.value = this.value.replace(/[^\d.]/g,'');"  :disabled="urlType" placeholder="请输入梨数量"></el-input> （亩）
                                </el-form-item>
                                <el-form-item label="葡萄" class="formItem">
                                    <el-input v-model="ruleForm.grapeArea" onkeyup="this.value = this.value.replace(/[^\d.]/g,'');"  :disabled="urlType" placeholder="请输入葡萄数量"></el-input> （亩）
                                </el-form-item>
...
<script>
//种植业股---果园面积 接口
import { orchardAreaDetail,orchardAreaUpdate, orchardAreaInsert } from '@/utils/api/planting/orchardArea' 
...
export default {
    ...
    watch:{
        ...
        testDataNew: {
            handler(newVal) {
                //总和
                this.ruleForm.sumArea = newVal.appleArea * 1 + newVal.grapeArea * 1 + newVal.peachArea * 1 +  newVal.pearArea * 1
           },
    ...
    methods:{
        ...
        async getDataDetail(){
            ...
            let res = await orchardAreaDetail({id:this.id});
        ...
        async updateData(){
            ...
            let res = await orchardAreaUpdate(updateObj);
             if(res.data.status== 0){
                this.$message({
                    message: '编辑成功',
                    type: 'success'
        ...
        async addData(){
            ...
            let res = await orchardAreaInsert(addObj);
             if(res.data.status== 0){
                this.$message({
                    message: '新增成功',
                    type: 'success'
