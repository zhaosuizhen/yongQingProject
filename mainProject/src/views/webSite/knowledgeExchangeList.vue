        ...
        <div class="webSite-content1-center" style="height: 100%;">
          ...
          <el-row class="tac">
            ...
            <el-col :span="20" style="margin-left: 2%">
                ...
                <div class="content" v-for="(item,index) in questionList" :key="index">
                    ...
                    <h1 style="margin-bottom:0px;text-align:left">{{item.title}}</h1>

                    <img width="40%" :src="list.titleImage" alt="">

                    <div style="margin-bottom:40px" class="question_comtent">
                        <span style="text-indent:0;float:left">具体描述：</span>
                        <span style="text-indent:0" v-html="item.content"></span>
                        <span style="line-height:30px">已回答 <b>{{item.answerCount}}</b> 条</span>
                        <el-button v-if="list.id" class="look_comment_btn" size="mini" type="success" @click="goTuInfoPage(item)">查看详情</el-button>
                    </div>
    ...
    methods:{
        ...
        async getPageList(currentPage){
            ...
            if(res.data.data.pageData.length){
                this.questionList = res.data.data.pageData
